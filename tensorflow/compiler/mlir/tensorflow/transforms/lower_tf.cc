 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *src_op,
    ...
    auto in_y_f32 = rewriter.create<CastOp>(
        loc, tensorflow::GetTypeFromTFTensorShape({}, rewriter.getF32Type()), in_y);
    auto out_y_f32 = rewriter.create<CastOp>(
        loc, tensorflow::GetTypeFromTFTensorShape({}, rewriter.getF32Type()), out_y);

    Value y_scale = rewriter.create<DivOp>(
        loc, tensorflow::GetTypeFromTFTensorShape({}, rewriter.getF32Type()), in_y_f32,
        out_y_f32);
    ...
    Value y_range = rewriter.create<RangeOp>(
        loc,
        tensorflow::GetTypeFromTFTensorShape({out_y_cst}, rewriter.getF32Type()),
        zero_f32, out_y_f32, one_f32);

    if (half_pixel_centers) {
      y_range = rewriter.create<AddV2Op>(
          loc, tensorflow::GetTypeFromTFTensorShape({out_y_cst}, rewriter.getF32Type()),
          y_range, half_f32);
    }

    y_range = rewriter.create<MulOp>(
        loc, tensorflow::GetTypeFromTFTensorShape({out_y_cst}, rewriter.getF32Type()), y_range,
        y_scale);

    y_range = rewriter.create<CastOp>(
        loc, tensorflow::GetTypeFromTFTensorShape({out_y_cst}, out_size_element_ty), y_range);

    y_range = rewriter.create<ReshapeOp>(
        loc, tensorflow::GetTypeFromTFTensorShape({out_y_cst, 1}, out_size_element_ty),
        y_range,
    ...
    auto in_x_f32 = rewriter.create<CastOp>(
        loc, tensorflow::GetTypeFromTFTensorShape({}, rewriter.getF32Type()), in_x);
    auto out_x_f32 = rewriter.create<CastOp>(
        loc, tensorflow::GetTypeFromTFTensorShape({}, rewriter.getF32Type()), out_x);

    Value x_scale = rewriter.create<DivOp>(
        loc, tensorflow::GetTypeFromTFTensorShape({}, rewriter.getF32Type()), in_x_f32,
        out_x_f32);

    Value x_range = rewriter.create<RangeOp>(
        loc, tensorflow::GetTypeFromTFTensorShape({out_x_cst}, rewriter.getF32Type()),
    ...
    x_range = rewriter.create<MulOp>(
        loc, tensorflow::GetTypeFromTFTensorShape({out_x_cst}, rewriter.getF32Type()), x_range,
        x_scale);

    x_range = rewriter.create<CastOp>(
        loc, tensorflow::GetTypeFromTFTensorShape({out_x_cst}, out_size_element_ty), x_range);
    ...
    Value indices = rewriter.create<AddV2Op>(
        loc, tensorflow::GetTypeFromTFTensorShape({out_y_cst, out_x_cst}, out_size_element_ty),
        y_indices, x_indices);
    ...
    Value gathered_values = rewriter.create<GatherV2Op>(
        loc,
        tensorflow::GetTypeFromTFTensorShape({batch_cst, out_y_cst, out_x_cst, channels_cst},
                              input_element_ty),
